...
#I104689: infiammato            con 2001 samples 
...
cells_to_rem <- getCells(obj)[getCellsSize(obj) > 7500]
...
cells_to_rem <- names(cellGeneNumber)[cellGeneNumber > 2250]
obj <- dropGenesCells(obj, cells = cells_to_rem)
genesSizePlot(obj)
...
plot2 <- UMAPPlot(t(obj@raw), clusters = mergedClusters, elements = markers$Gene, title = "Merged Cluster UMAP Plot")
plot(plot2)

treePlot = clustersTreePlot(obj, kCuts = 6, clName = "FineClusters" )
plot(treePlot$dend)

treePlot = clustersTreePlot(obj, kCuts = 6, clName = "MergedClusters" )
plot(treePlot$dend)
...
'
c(clustersData, clustersDataCoex) %<-% getClusterizationData(obj, "MergedClusters")
n_clusters = unique(clustersData)

i = 1
for(cluster in n_clusters)
{
  currClusterCoex = coexDF[[i]]
  geneSetEnrichment(clustersCoex = currClusterCoex, groupMarkers = groupMarkers)
